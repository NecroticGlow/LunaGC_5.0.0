...
public class OpeConfigFromSp {
    ...
    public void saveGlobalWaterMarkConfig(Context context, WaterMarkConfig config) {
        if(config == null) return;
        
        config.packageList = null; // 全局配置不限制包名
        saveWaterMarkConfig(context, config);
    }

    public WaterMarkConfig getGlobalWaterMarkConfig(Context context) {
        List<WaterMarkConfig> configs = getWaterMarkConfigs(context);
        for (WaterMarkConfig config : configs) {
            if (config.isOpen() && (config.packageList == null || config.packageList.isEmpty())) {
                return config;
            }
        }
        return new WaterMarkConfig();
